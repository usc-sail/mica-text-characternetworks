...
speakerInfoDir = '../../BiasUsingText/v2/Data/charandmovie_info/'
...
th=1
filename=sys.argv[1]
filepath=os.path.join(speakerInfoDir, filename.split('/')[-1])

speakerFilePtr = open(filepath, 'r')

speakerInfoLines = speakerFilePtr.readlines()

speakerInfo = defaultdict(lambda:'unknown')

for currentLine in speakerInfoLines[28:]:
    ...
    if len(tmpList2) == 7:
        ...
        gender = tmpList2[-3].strip()
        genderConfidence = float(tmpList2[-2].strip())
        race = tmpList2[-1].strip()
    else:
        numUtterances = int(tmpList2[0].strip())
    ...
    if speakerName not in speakerInfo.keys():
        speakerInfo[speakerName] = [numUtterances, gender, genderConfidence, race]

spkTransition=defaultdict(lambda:defaultdict(lambda:[0, []]))

inptr=open(filename)
dlgs=[line.strip() for line in inptr.readlines()]
inptr.close()

lastSpeaker=''
for d in dlgs:
    tmplist=[elem.strip() for elem in d.split('=>')]
    thisSpeaker=tmplist[0]
    utt=tmplist[1]
    if thisSpeaker not in speakerInfo.keys():
        continue

    charGender = speakerInfo[clean_speaker(thisSpeaker)][1]
    ...
    if len(lastSpeaker)>0:
        ...
        if charGender != 'male' and charGender != 'unknown':
            spkTransition[lastSpeaker][thisSpeaker][1].append(utt)
            spkTransition[thisSpeaker][lastSpeaker][1].append(utt)
...
for char in char_list:
    if bechdel_pair != '' and char in bechdel_pair[0]:
        ...
        bechdel_node=0
    char_nodes_list.append({"name": char, "sno": counter, "count": speakerInfo[char][0], "gender": speakerInfo[char][1], "race": speakerInfo[char][3], "bechdel": bechdel_node})
    counter = counter + 1
